...
export function validateContent(raw){
 ...
 str(d.projects.heading);d.projects.items??=[];list(d.projects.items,0,100);const projectIds=new Set();d.projects.items.forEach(x=>{str(x.name,200);str(x.id||'',100);assert(/^[a-z0-9][a-z0-9-]{2,80}$/.test(x.id||''),'Dự án thiếu mã ổn định.');assert(!projectIds.has(x.id),'Mã dự án bị trùng.');projectIds.add(x.id);link(x.image,true);drive(x.fullVideo);});
