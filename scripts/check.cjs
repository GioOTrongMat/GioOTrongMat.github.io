...
for(const asset of [d.logo,d.about.image,...d.works.items.map(w=>w.image),...d.projects.items.map(p=>p.image)].filter(Boolean))if(asset.startsWith('/')&&!asset.startsWith('/uploads/')){const parts=asset.slice(1).split('/');let base=root;for(const part of parts){assert.ok(fs.readdirSync(base).includes(part),`Asset case mismatch or missing: ${asset}`);base=path.join(base,part);}}
